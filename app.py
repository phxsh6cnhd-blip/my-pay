...
if menu == "🚀 실시간 대시보드":
    ...
    if is_working:
        ...
        if st.button("🚨 퇴근하고 기록 저장"):
            os.remove(TEMP_FILE)
            st.success("퇴근 기록 완료! 정산소에서 확인하세요.")
            st.balloons()
            st.rerun()
...
# --- 2. 주급 정산소 화면 ---
    ...
    st.title("🧾 주급 정산소")
    st.write("여기서 이번 주 일한 기록을 확인하고 정산하세요!")
    
    # 기존에 만들었던 데이터 테이블과 정산 로직이 들어갈 자리입니다.
    data_df = pd.read_csv(DATA_FILE)
    if data_df.empty:
        st.warning("아직 정산된 기록이 없습니다.")
    else:
        st.dataframe(data_df, use_container_width=True)
    
    if st.button("🗑️ 모든 기록 삭제"):
        data_df.drop(data_df.index, inplace=True)
